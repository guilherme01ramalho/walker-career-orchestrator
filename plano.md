...
**Escopo**: Implementar o bloco Scout e conectá-lo ao Walker.
...
**Saídas**:
- Retornar resultados ao Walker via Response Envelope (Walker salva em `data/job-search-results.md`)
...
**Envelope de Despacho** (construído pelo Walker):
...
### 3. Conectar `spawn_agent` no Walker para despachar o Scout

- Quando o usuário digitar "A", o Walker deve construir o envelope de despacho
...
Usuário seleciona A no menu
        │
        ▼
Walker constrói envelope de despacho para Scout
        │
        ▼
...
Scout retorna envelope de resposta com até 5 vagas
        │
        ▼
Walker salva em data/job-search-results.md → exibe ao usuário → retorna ao menu
```
